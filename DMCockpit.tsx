...
export default function DMCockpit() {
  ...
  const [editMax,  setEditMax]  = useState(null); // id of player editing max HP
  const [editMaxE, setEditMaxE] = useState(null); // id of enemy editing max HP
  ...
  const [tipDie,    setTipDie]    = useState(null);
  ...
  const [slotMeta,      setSlotMeta]      = useState(Array.from({length:NUM_SLOTS},(_,i)=>({index:i,empty:true})));
  ...
  return (
    <>
      ...
      <div className="root">
        ...
        {/* TOAST */}
        {toastMsg&&(
          <div className={`save-toast ${toastMsg.hiding?"hiding":""}`} style={{borderColor:toastMsg.color+"66",color:toastMsg.color}}>
            {toastMsg.text}
          </div>
        )}
