...
.map-toolbar{display:flex;align-items:center;gap:8px;padding:8px 12px;background:rgba(12,7,3,.95);border-bottom:1px solid var(--border);flex-wrap:wrap;}
.map-tool-btn{padding:6px 10px;border-radius:5px;border:1px solid rgba(201,162,39,.25);background:rgba(201,162,39,.05);color:rgba(201,162,39,.6);font-family:'Cinzel',serif;font-size:.58rem;cursor:pointer;transition:all .15s;}
.map-tool-btn:hover{background:rgba(201,162,39,.12);color:var(--gold);}
.map-tool-btn.active{background:rgba(201,162,39,.2);border-color:var(--gold);color:var(--gold);}
.map-tool-sep{width:1px;height:24px;background:rgba(201,162,39,.2);margin:0 4px;}
.map-toggle{display:flex;align-items:center;gap:6px;font-family:'Cinzel',serif;font-size:.55rem;color:rgba(201,162,39,.6);}
.map-toggle input[type="checkbox"]{accent-color:var(--gold);}
.map-canvas-wrap{flex:1;overflow:hidden;position:relative;background:radial-gradient(ellipse at center,#1a0e06 0%,#0a0402 100%);cursor:grab;}
.map-canvas-wrap.dragging{cursor:grabbing;}
.map-svg{width:100%;height:100%;}
.map-token{cursor:grab;transition:transform .1s;}
.map-token:hover{transform:scale(1.15);}
.map-token:active{cursor:grabbing;transform:scale(1.2);}
.map-token-selected{filter:drop-shadow(0 0 8px var(--gold));}
.map-territory{opacity:.6;}
.map-territory-path{stroke:rgba(201,162,39,.4);stroke-width:2;fill:rgba(201,162,39,.1);}
.map-territory-label{font-family:'Cinzel',serif;font-size:.6rem;fill:var(--gold);text-anchor:middle;pointer-events:none;}
.map-grid-line{stroke:rgba(201,162,39,.08);stroke-width:1;}
.map-compass{position:absolute;bottom:16px;right:16px;width:60px;height:60px;opacity:.6;pointer-events:none;}
.map-zoom-indicator{position:absolute;bottom:16px;left:16px;font-family:'Cinzel',serif;font-size:.55rem;color:rgba(201,162,39,.5);background:rgba(12,7,3,.8);padding:4px 8px;border-radius:4px;border:1px solid rgba(201,162,39,.15);}
.map-token-lib{display:flex;gap:6px;padding:6px 12px;background:rgba(12,7,3,.9);border-top:1px solid var(--border);overflow-x:auto;}
.map-token-preview{width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:1.2rem;cursor:grab;flex-shrink:0;transition:transform .1s;}
.map-token-preview:hover{transform:scale(1.2);}
...
export default function DMCockpit() {
  ...
  const [mapState, setMapState] = useState<MapState>({
    ...
    drawingTerritoryPoints: undefined,
  });

  // Map interaction state
  const [isDragging, setIsDragging] = useState(false);
  const [dragStart, setDragStart] = useState({ x: 0, y: 0 });
  const [isDraggingToken, setIsDraggingToken] = useState<number | null>(null);
  ...
  useEffect(() => {
    stateRef.current = { players, enemies, log, chapters, activeChapterId, diceHist: diceHist || [], combat, order, turn, mapState };
  });
  ...
  const TOKEN_ICONS = ["⚔️", "🔮", "🗡️", "✝️", "🏹", "🛡️", "🪓", "🎵", "🌿", "👊", "✨", "👁️", "👺", "👹", "💀", "🧟", "🦹", "🐺", "🐉", "👾", "🧛", "☠️", "🗿", "❓", "🧙", "🧝", "🧛‍♂️", "🧟‍♂️", "👹", "⚡", "🔥", "💎", "📦", "🏕️", "🏰", "🌲", "🏔️", "🌊"];
  const TOKEN_COLORS = ["#c9a227", "#e03030", "#30a030", "#3060e0", "#a030a0", "#e08030", "#30a0a0"];

  const addMapToken = (icon: string, x?: number, y?: number) => {
    ...
    setMapState(prev => ({ ...prev, tokens: [...prev.tokens, token], selectedTokenId: token.id }));
  };

  const updateMapToken = (id: number, updates: Partial<MapToken>) => {
    setMapState(prev => ({
      ...prev,
  ...
  const removeMapToken = (id: number) => {
    setMapState(prev => ({ ...prev, tokens: prev.tokens.filter(t => t.id !== id), selectedTokenId: undefined }));
  };

  const addTerritory = (points: { x: number, y: number }[]) => {
  ...
  const removeTerritory = (id: number) => {
    setMapState(prev => ({ ...prev, territories: prev.territories.filter(t => t.id !== id) }));
  };

  const setMapTool = (tool: MapState['drawingTool']) => {
    setMapState(prev => ({ ...prev, drawingTool: tool, drawingTerritoryPoints: undefined }));
  };

  const toggleGrid = () => {
    setMapState(prev => ({ ...prev, gridVisible: !prev.gridVisible }));
  };

  const zoomMap = (delta: number) => {
    setMapState(prev => ({ ...prev, zoom: Math.max(0.5, Math.min(3, prev.zoom + delta)) }));
  };

  const mapCanvasRef = useRef<SVGSVGElement>(null);

  const getMapCoords = (e: React.MouseEvent | React.TouchEvent) => {
    if (!mapCanvasRef.current) return { x: 0, y: 0 };
    const rect = mapCanvasRef.current.getBoundingClientRect();
    const clientX = 'touches' in e ? e.touches[0].clientX : (e as React.MouseEvent).clientX;
    const clientY = 'touches' in e ? e.touches[0].clientY : (e as React.MouseEvent).clientY;
    return {
      x: (clientX - rect.left - mapState.offsetX) / mapState.zoom,
      y: (clientY - rect.top - mapState.offsetY) / mapState.zoom,
    };
  };

  const handleCanvasMouseDown = (e: React.MouseEvent | React.TouchEvent) => {
    if (mapState.drawingTool === 'select') {
      setIsDragging(true);
      const pos = getMapCoords(e);
      setDragStart({ x: pos.x - mapState.offsetX / mapState.zoom, y: pos.y - mapState.offsetY / mapState.zoom });
    } else if (mapState.drawingTool === 'territory') {
      const pos = getMapCoords(e);
      setMapState(prev => ({
        ...prev,
        drawingTerritoryPoints: [...(prev.drawingTerritoryPoints || []), pos],
      }));
    } else if (mapState.drawingTool === 'token') {
      const pos = getMapCoords(e);
      addMapToken(TOKEN_ICONS[0], pos.x, pos.y);
    }
  };
  ...
  const handleCanvasMouseMove = (e: React.MouseEvent | React.TouchEvent) => {
    if (isDragging) {
      ...
      setMapState(prev => ({
        ...prev,
        offsetX: (pos.x - dragStart.x) * prev.zoom,
        offsetY: (pos.y - dragStart.y) * prev.zoom,
      }));
    } else if (isDraggingToken !== null) {
      const pos = getMapCoords(e);
      updateMapToken(isDraggingToken, { x: pos.x, y: pos.y });
    }
  };
  ...
  const handleTokenMouseDown = (e: React.MouseEvent | React.TouchEvent, tokenId: number) => {
    e.stopPropagation();
    if (mapState.drawingTool === 'select') {
      setMapState(prev => ({ ...prev, selectedTokenId: tokenId }));
      setIsDraggingToken(tokenId);
    } else if (mapState.drawingTool === 'eraser') {
      removeMapToken(tokenId);
    }
  };

  const handleTerritoryComplete = () => {
    if (mapState.drawingTerritoryPoints && mapState.drawingTerritoryPoints.length >= 3) {
      addTerritory(mapState.drawingTerritoryPoints);
    }
  };

  const mapContent = () => {
    const { tokens, territories, gridVisible, gridSize, zoom, offsetX, offsetY, drawingTool, selectedTokenId, drawingTerritoryPoints } = mapState;

    // Generate grid lines
    const gridLines = [];
    if (gridVisible) {
      const gridSizeScaled = gridSize * zoom;
      const offsetXMod = offsetX % gridSizeScaled;
      const offsetYMod = offsetY % gridSizeScaled;
      for (let i = -10; i < 50; i++) {
        const x = i * gridSizeScaled + offsetXMod;
        const y = i * gridSizeScaled + offsetYMod;
        gridLines.push(<line key={`vx${i}`} x1={x} y1={-500} x2={x} y2={1500} className="map-grid-line" />);
        gridLines.push(<line key={`hz${i}`} x1={-500} y1={y} x2={1500} y2={y} className="map-grid-line" />);
      }
    }
    ...
    return (
      <div className="map-container">
        ...
        <div className="map-toolbar">
          <button className={`map-tool-btn ${drawingTool === 'select' ? 'active' : ''}`} onClick={() => setMapTool('select')}>👆 Select</button>
          <button className={`map-tool-btn ${drawingTool === 'token' ? 'active' : ''}`} onClick={() => setMapTool('token')}>➕ Token</button>
          <button className={`map-tool-btn ${drawingTool === 'territory' ? 'active' : ''}`} onClick={() => setMapTool('territory')}>🖊️ Territory</button>
          <button className={`map-tool-btn ${drawingTool === 'eraser' ? 'active' : ''}`} onClick={() => setMapTool('eraser')}>🧹 Eraser</button>
          <div className="map-tool-sep" />
          <button className="map-tool-btn" onClick={() => zoomMap(-0.1)}>-</button>
          <button className="map-tool-btn" onClick={() => zoomMap(0.1)}>+</button>
          ...
          <div className="map-tool-sep" />
          {drawingTool === 'territory' && (
            <button className="map-tool-btn" onClick={handleTerritoryComplete} style={{ borderColor: 'var(--gold)', color: 'var(--gold)' }}>
              ✓ Finish Territory ({drawingTerritoryPoints?.length || 0} pts)
            </button>
          )}
          {selectedTokenId !== undefined && (
            <button className="map-tool-btn" onClick={() => removeMapToken(selectedTokenId)} style={{ borderColor: '#e03030', color: '#e03030' }}>
              🗑️ Remove Token
            </button>
          )}
        </div>
        ...
        {/* Map Canvas */}
        <div
          className={`map-canvas-wrap ${isDragging || isDraggingToken !== null ? 'dragging' : ''}`}
        ...
        >
          <svg ref={mapCanvasRef} className="map-svg" viewBox="0 0 800 600">
            <g transform={`translate(${offsetX}, ${offsetY}) scale(${zoom})`}>
              ...
              {territories.map(territory => {
                const pathData = territory.points.map((p, i) => `${i === 0 ? 'M' : 'L'} ${p.x} ${p.y}`).join(' ') + ' Z';
                const centroidX = territory.points.reduce((s, p) => s + p.x, 0) / territory.points.length;
                const centroidY = territory.points.reduce((s, p) => s + p.y, 0) / territory.points.length;
                return (
                  <g key={territory.id} className="map-territory">
                    <path d={pathData} className="map-territory-path" fill={territory.filled ? `${territory.color}22` : 'none'} stroke={territory.color} />
                    <text x={centroidX} y={centroidY} className="map-territory-label">{territory.label}</text>
                  </g>
                );
              })}

              {/* Drawing territory preview */}
              {drawingTerritoryPoints && drawingTerritoryPoints.length > 0 && (
                <path
                  d={drawingTerritoryPoints.map((p, i) => `${i === 0 ? 'M' : 'L'} ${p.x} ${p.y}`).join(' ')}
                  fill="none"
                  stroke="var(--gold)"
                  strokeWidth="2"
                  strokeDasharray="5,5"
              ...
              {tokens.map(token => (
                <g
                  key={token.id}
                  className={`map-token ${selectedTokenId === token.id ? 'map-token-selected' : ''}`}
                  transform={`translate(${token.x - token.size / 2}, ${token.y - token.size / 2})`}
                  onMouseDown={(e) => handleTokenMouseDown(e, token.id)}
                  onTouchStart={(e) => handleTokenMouseDown(e, token.id)}
                >
                  <circle cx={token.size / 2} cy={token.size / 2} r={token.size / 2} fill={`${token.color}33`} stroke={token.color} strokeWidth="2" />
                  <text x={token.size / 2} y={token.size / 2 + 4} textAnchor="middle" fontSize={token.size * 0.6} dominantBaseline="middle">{token.icon}</text>
                  {token.label && (
                    <text x={token.size / 2} y={token.size + 8} textAnchor="middle" fontSize="10" fill="var(--gold)">{token.label}</text>
                  )}
                </g>
              ))}
            </g>
          ...
          {/* Compass */}
          <svg className="map-compass" viewBox="0 0 60 60">
            <circle cx="30" cy="30" r="28" fill="none" stroke="rgba(201,162,39,.3)" strokeWidth="1" />
            <polygon points="30,8 35,25 30,30 25,25" fill="rgba(201,162,39,.6)" />
            <polygon points="30,52 35,35 30,30 25,35" fill="rgba(201,162,39,.3)" />
            <text x="30" y="7" textAnchor="middle" fill="rgba(201,162,39,.7)" fontSize="8" fontFamily="serif">N</text>
          </svg>

          {/* Zoom indicator */}
          <div className="map-zoom-indicator">{Math.round(zoom * 100)}%</div>
        </div>

        {/* Token Library */}
        <div className="map-token-lib">
          {TOKEN_ICONS.slice(0, 20).map((icon, i) => (
            <div
              key={i}
              className="map-token-preview"
              style={{ background: `${TOKEN_COLORS[i % TOKEN_COLORS.length]}33`, border: `2px solid ${TOKEN_COLORS[i % TOKEN_COLORS.length]}` }}
              onClick={() => addMapToken(icon)}
              title={`Add ${icon}`}
            >
              {icon}
            </div>
  ...
  const chroniclesContent = () => {
    ...
    const addChapter = () => {
      ...
      const newCh = {
        id,
        name: chapters.length === 0 ? "Prologue" : `Chapter ${chCount + 1}`,
        summary: "",
    ...
    if (!activeCh) {
      return (
        <div style={{ padding: "40px 20px", textAlign: "center", display: "flex", flexDirection: "column", gap: "16px", alignItems: "center" }}>
          <div className="lempty" style={{ padding: 0 }}>The Grimoire is currently blank.</div>
          <button className="cbtn btn-gld" style={{ maxWidth: "200px" }} onClick={addChapter}>
            Begin the Tale
          </button>
        </div>
      );
